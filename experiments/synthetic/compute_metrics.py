...
def main(cfg: DictConfig) -> None:
    ...
    for filename in sorted(glob.glob(PATH_LOAD + "*")):
        ...
        for method in f1_dict.keys(): 
            ...
            dic_metrics.append({"number of edges": s, "f1": f1, "method": method, "precision": precision, "recall": recall })
    ...
    for method in ["auto", "pca", "gt", "notears"]:
        ...
        for s in [10, 20, 30, 40]:
            ...
            print("f1", round(np.mean(x_gt["f1"]),2), round(1.96/np.sqrt(len(x_gt))*np.std(x_gt["f1"]),2))
            print("precision", round(np.mean(x_gt["precision"]),2), round(1.96/np.sqrt(len(x_gt))*np.std(x_gt["precision"]), 2))
            print("recall", round(np.mean(x_gt["recall"]), 2), round(1.96/np.sqrt(len(x_gt))*np.std(x_gt["recall"]), 2))

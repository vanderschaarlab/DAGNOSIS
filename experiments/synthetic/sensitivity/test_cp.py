...
def main(cfg: DictConfig):
    ...
    for filename in glob.glob(PATH_LOAD + "*"):
        ...
        corrupted_dag_dict = dic_data["corrupted_dags_dict"] #dictionary with key the SHD, and value the list of corrupted dags

        dic_SHD = {} #dictionnary with key the SHD, and value the list of conformal estimators (one for each corrupted dag)
        list_conf_gt, list_conf_pca, list_conf_dagma  = dic_results["conf_gt"], dic_results["conf_pca"], dic_results["conf_dagma"]
        ...
        for run in range(n_runs):
            ...
            X_test_corrupted, _, _ =sample_corrupted(D_train, n_corrupted, list_features_corruption, list_corruption_type, noise_mean_list=noise_mean_list , 
                                                     sample_last_layer=False, mean_mlp = 0, std_mlp = 0.3)
            

            TP_gt, conf_dict_gt = compute_TP(list_conf_gt, A_gt, X_test_corrupted)
            TP_dagma, conf_dict_dagma = compute_TP(list_conf_dagma, A_dagma, X_test_corrupted)

            TP_pca, conf_dict_pca = compute_TP_pca(list_conf_pca, scaler,pca, X_test_corrupted)
            ...
            for SHD in list_SHD:
                ...
                for i in range(len(dic_SHD[SHD])):
                    ...
                    TP, _ = compute_TP(list_conf_corrupted, corrupted_dag, X_test_corrupted)
            ...
            k= 0
            while str(os.path.join(PATH_SAVE_confdict, f"run_{k}_{id}" )) in glob.glob(PATH_SAVE_confdict + "*"):
                k+=1 
            
            dic_conf = {"gt": conf_dict_gt, "pca": conf_dict_pca, "dagma": conf_dict_dagma}
        ...
        for method in dic_recall.keys():
            dic_f1[method].append(2/(1/np.array(dic_precision[method]) + 1/np.array(dic_recall[method])))

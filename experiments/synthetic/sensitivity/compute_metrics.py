...
def main(cfg: DictConfig) -> None:
    ...
    for method in ["pca", "dagma", "SHD_10", "SHD_20", "SHD_30", "SHD_40"]:
        ...
        print(
            "f1",
            round(np.mean(x["f1"]), 2),
            round(1.96 / np.sqrt(len(x)) * np.std(x["f1"]), 2),
        )
        print(
            "precision",
            round(np.mean(x["precision"]), 2),
            round(1.96 / np.sqrt(len(x)) * np.std(x["precision"]), 2),
        )
        print(
            "recall",
            round(np.mean(x["recall"]), 2),
            round(1.96 / np.sqrt(len(x)) * np.std(x["recall"]), 2),
        )

import numpy as np
from src.utils.conformal import analyse_conformal_dict, predict_cp, predict_cp_pca
...
def compute_TP_pca(list_conf, scaler, pca, X_test_corrupted):
    ...
    conformal_dict_corrupted = predict_cp_pca(X_test_corrupted, list_conf, np.arange(d), scaler, pca)
    df_pca = analyse_conformal_dict(conformal_dict_corrupted)
    TP_pca = len(df_pca[df_pca['inconsistent'] == True])
